...
def get_clean_num(num, to_check=None):
    if to_check is False:
        pu_num = '0'
        try:
            pu_num_str = re.sub(r'\D', '', num)
            pu_num = int(pu_num_str)
...
def get_area(data):
    ...
    for area_entry in data:
        if area_entry:
            ...
            if 'Троицкий' in area_entry:
                area_entry = 'Троицкий РЭС'
...
def gen_fiz_keys():
    ...
    result = db.qdb(r"""SELECT accounting_num, pu_num, id,
    ...
    if result:
        for row in result:
            ...
            if type(row[4]) == datetime.date:
                datapk1 = row[4].strftime("%Y-%m-%d")
            elif row[4] is None:
                pass

            if type(row[5]) == datetime.date:
                datapk2 = row[5].strftime("%Y-%m-%d")
            elif row[5] is None:
                pass
            # по новой логике ls
            ...
            if ls_int not in work_dicts['fiz_keys']:
                ...
                work_dicts['fiz_keys'][ls_int][pu_num_int] = {'data': [row[2], row[3], datapk1, datapk2,
                                                                       row[6], row[7]],
                                                              'repeat': 0,
                                                              'is_inserted': 0,
                                                              'updated': 0}
            # если есть такой ls_int
                ...
                if pu_num_int not in work_dicts['fiz_keys'][ls_int]:
                    work_dicts['fiz_keys'][ls_int][pu_num_int] = {
                        'data': [row[2], row[3], datapk1, datapk2, row[6], row[7]],
...
# Проверка значения в excel(является ли значение заголовокм типа NPEC)
